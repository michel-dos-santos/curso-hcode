class DropBoxController {
    constructor(){
        ...
        this.filename = this.snackModal.querySelector('.filename');
    ...
    connectFirebase(){
        var config = {
            apiKey: "AIzaSyC9cYUr0VLoMFIedUWOxW-p2Or-liJqdqk",
            authDomain: "dropbox-clone-27b4b.firebaseapp.com",
            databaseURL: "https://dropbox-clone-27b4b.firebaseio.com",
            projectId: "dropbox-clone-27b4b",
            storageBucket: "dropbox-clone-27b4b.appspot.com",
            messagingSenderId: "170858104922"
        };
    ...
    removeFile(ref, name){
        return firebase.storage().ref(ref).child(name).delete();
    }
    ...
    initEvents(){
        ...
        this.listFiles.addEventListener('selectionChange', event => {
            ...
            this.uploadTask(event.target.files).then(responses => {
                ...
                responses.forEach(resp => {
                    this.getFirebaseRef().push().set({
                        ...
                        path: `https://firebasestorage.googleapis.com/v0/b/dropbox-clone-27b4b.appspot.com/o/${resp.fullPath.replace("/", "%2F")}?alt=media`,
                        size: resp.size
                    });
                });
                this.uploadComplete();
    ...
    uploadProgress(event, file){
        ...
        let porcent = (loaded * 100) / total;
        let timeleft = ((100 - porcent) * timespent) / porcent

        this.progressBar.style.width = `${parseInt(porcent)}%`
    ...
    formatTimeToHuman(duration){
        ...
        let message = `${seconds} segundos`;

        if(hours > 0){
            message = `${hours} horas, ${minutes} minutos e ${seconds} segundos`;
        }else if(minutes > 0){
            message = `${minutes} minutos e ${seconds} segundos`;
        }
        return message;
    }
    ...
    initEventLI(li){
        ...
        li.addEventListener('click', event => {
            if(event.shiftKey){
               let firstLI = this.listFiles.querySelector('li.selected');
               if(firstLI){
                    let indexStart;
                    let indexEnd;
            ...
            if(!event.ctrlKey){
                this.listFiles.querySelectorAll('li.selected').forEach(element => {
